...
public class PlayerControl : MonoBehaviour
{
    ...
    public Vector2 playerDirection = Vector3.zero;
    ...
    {
        if (GetInput().x < 0) playerDirection = Vector2.left;
        if (GetInput().x > 0) playerDirection = Vector2.right;
        if (GetInput().y < 0) playerDirection = Vector2.down;
        if (GetInput().y > 0) playerDirection = Vector2.up;
    }
}